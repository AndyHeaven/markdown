...
通过service-edu模块调用service-vod模块的删除视频方法
...
4. 最终存入redis的key是@Cacheable注解中的key::value
...
![在这里插入图片描述](/Users/zhy/谷粒学院资料/SSO.png)
...
采用@GetMapping搭配@RequestBody出错，因为@RequestBody主要用来接收前端传递给后端的json字符串中的数据的(请求体中的数据的)；GET方式无请求体（近乎无用），所以使用@RequestBody接收数据时，前端不能使用GET方式提交数据，而是用POST方式进行提交。在后端的同一个接收方法里，@RequestBody与@RequestParam()可以同时使用，@RequestBody最多只能有一个，而@RequestParam()可以有多个
